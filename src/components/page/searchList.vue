  ...
  <div class="search-list">
    <div class="search-result-list">
      <el-card class="search-result-card" shadow="hover" v-for="(item, index) in list" :key="index">
        <div slot="header" class="search-result-title" @click="go(item.id)">
          <a class="title">{{item.disputeName}}</a>
        </div>
...
.searchList .search-tool-contain{position:fixed;top:60px;padding:25px 0;width:100%;background:rgba(25, 25, 25, 0.60);z-index: 1;}
...
.searchList .search-list{position: relative;padding: 180px 50px 50px 50px;box-sizing: border-box;max-height: 100%;overflow: auto;z-index: 0;}
...
.searchList .search-result-list .search-result-info{padding-bottom:6px;border-bottom:1px dotted #eaeaea;max-width:100%;text-overflow:ellipsis;overflow:hidden;white-space:nowrap;color:#db2929;text-decoration:none;}
.searchList .search-result-list .search-result-info:active{text-decoration: none;}
.searchList .search-result-list .search-result-text{padding-top:6px;position:relative;line-height:25px;max-height:75px;width:100%;display: -webkit-box;-webkit-box-orient: vertical;-webkit-line-clamp: 3;overflow: hidden;}
